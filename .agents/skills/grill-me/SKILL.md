...
Read `AGENTS.md` and `openspec/config.yaml` before starting. Follow the user's stated scope. For nontechnical teammates, ask about users, behavior, exclusions, and how they will recognize success. Explain technical trade-offs only when they change those decisions.
...
Finish with a short record of the agreed outcome, scope, exclusions, acceptance examples, and unresolved questions. Keep it in the conversation unless the user asks to save it. When asked for a plan, use the installed `openspec-propose` skill to capture these decisions in an OpenSpec change. Do not repeat settled questions. Nontechnical teammates review the behavior; engineering owns the technical design and implementation.
