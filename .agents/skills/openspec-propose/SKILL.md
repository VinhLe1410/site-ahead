...
2. **Load project context**
   ...
   Run `npm run --silent openspec -- context --json` from the current working directory (or `npm run --silent openspec -- context --json --store "<store-id>"` when a registered store was explicitly selected). Use the returned `root.path` as the authoritative OpenSpec root. If context reports `no_openspec_root`, stop without creating or changing any files. Offer `npm run openspec:refresh` and wait for the user to request initialization. Do not initialize automatically or run `npm run --silent openspec -- new change`. After initialization, rerun this context check before continuing. For any other context failure, stop and report the error; do not fall back to the current directory or run later OpenSpec commands without the selected store.
...
3. **Determine the workflow schema**
   ...
   **Use a different schema only if the user:**
   ...
   - Asks to "show workflows" or asks "what workflows" exist → resolve the authoritative root by running `npm run --silent openspec -- context --json` from the current working directory. If the user explicitly selected a registered store, use `npm run --silent openspec -- context --json --store "<store-id>"`. Then run `npm run --silent openspec -- schemas --json` with its working directory set to the returned `root.path` and let them choose. This preserves roots selected by a local `store:` pointer or the global `defaultStore`; when a registered store was explicitly selected, append `--store "<store-id>"` to `npm run --silent openspec -- schemas --json` as well. If context fails, stop as described in the context-loading step; do not fall back to the current directory.
...
4. **Create the change directory**
   ...
   ```bash
   npm run --silent openspec -- new change "<name>"
   ```

   Using an explicitly requested schema:
   ```bash
   npm run --silent openspec -- new change "<name>" --schema "<schema-name>"
   ```
...
6. **Create every artifact in the required set**
   ...
   b. **Continue until every artifact in the required set exists (not just `apply.requires`)**
      - After creating each artifact, re-run `npm run --silent openspec -- status --change "<name>" --json`
...
7. **Show final status**
   ```bash
   npm run --silent openspec -- status --change "<name>"
   ```
...
- Follow the `instruction` field from `npm run --silent openspec -- instructions` for each artifact type - it is the authoritative guidance, even for familiar artifact names
