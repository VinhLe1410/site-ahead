...
4. **For each delta spec, apply changes to main specs**
   ...
   - If archive invoked this workflow inline and supplied a valid snapshot from `npm run --silent openspec -- instructions specs --change "<name>" --json`, reuse it and do not fetch the same instructions again.
   ...
   **`## Purpose` in the delta:**
   - The main spec already has one and it is authoritative - leave it alone (this is what `npm run --silent openspec -- archive` does; it warns and moves on)

   d. **Create new main spec** if capability doesn't exist yet:
   - Create `<planningHome.root>/openspec/specs/<capability-path>/spec.md`
   - Add Purpose section: copy the delta's `## Purpose` body verbatim when it has one (this is what `npm run --silent openspec -- archive` does); only write a brief TBD placeholder when it does not
   - Add Requirements section with the ADDED requirements
   - Follow the **Main Spec Format Reference** below

5. **Validate updated main specs**

   Run `npm run --silent openspec -- validate --specs` with the same selected-root flags used earlier. If validation fails, report the problems and do not claim the sync succeeded.
...
- A MODIFIED block carries the whole requirement - body plus every scenario that survives the change. `npm run --silent openspec -- validate` and `npm run --silent openspec -- archive` both reject one that drops a scenario the main spec still has.
