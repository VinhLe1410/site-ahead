...
- Started Google sign-in on every deployment and confirmed the redirect to Google uses that deployment's callback. The developer later confirmed the browser demo works; separate browser runs on every deployment were not recorded.
...
The developer confirmed the owner/staff browser demo works on 2026-09-13, completing task 5.4. The demo covers shared jobs and categories, owner-only controls, invitation expiry and revocation, live staff removal, account switching, and return destinations.
