...
## Step 4: Narrowed scope (buildable in ~24 hours — time already used, this is what's left)

Build the generic engine and prove it flexes across job types, not a fully-automated version of any single one. Stack: Convex for the backend (checklist data model, status mutations, storage), React + Vite for the front end, Tailwind for styling. Demo data is fully mocked — a fictional contractor ("Ironbark Site Services") and fictional but realistic-looking job addresses, with canned/seeded results standing in for the Automated Checks rather than live API calls. That removes any dependency on a public API responding correctly at demo time; the pitch still cites the real, confirmed public data sources (EPA AirWatch, Transport Victoria) as what the architecture is built to call in production. Priority order:

1. **Job-type → checklist data model** (highest priority, everything else depends on it): a Convex table of {job type → checklist item → category → trigger tag}, covering at least Excavation & Trenching and Electrical Work (add Plumbing/Renovation only if time remains). This is what proves "different checklist per job," and it's pure data, not integration work.
2. **Client-communication intake**: a paste-text box, plus a voice recorder wired to ElevenLabs Scribe for speech-to-text (this is the one genuinely live external API call in this build — controlled by the team reading a script, so far lower demo-day risk than depending on a public government API). Both feed one LLM extraction call that returns a job subtype and a list of triggered items, which is how the checklist gets tailored per client, not just per job type.
3. **The checklist control table UI** (React + Tailwind): item, category, status, action, with manual buttons to move status forward (mark sent, mark received, mark complete). Reads and writes through Convex functions, driven entirely by the data model regardless of category — the table doesn't care whether a status changed because of a seeded result or a button click.
4. **Automated Checks, seeded not live**: the threshold/classification-style ones (job value vs $750/$10k/$16k, prescribed vs non-prescribed) are just a rule, no external call needed. The API-style ones (air quality, traffic disruptions) use realistic canned responses for the demo rather than a live call.
5. **Third-Party Request drafting**: one LLM prompt template that drafts the relevant form/enquiry text per item (BYDA enquiry, inspection booking, compliance certificate submission). Sending is a manual UI action that flips status to Pending; receiving a reply is also marked manually via a button. Real inbox tracking (Gmail watch) is a roadmap idea, no time allocated to it here.
6. **On-Site Check items**: a checkbox and a notes field. Trivial, don't over-invest here.
7. **LLM report synthesis**: turn the resolved checklist state into a short readable brief at the end.
8. **Deploy to a judge-accessible URL** (Vite build, hosted wherever Convex's deploy target points).

Cut entirely for this window: any live external API call other than ElevenLabs Scribe, live Gmail/email watching, voice Q&A over the finished report (a different feature from the Scribe intake above — see Step 5), more than two fully-wired job types, anything resembling a CRM or inbox. If the data model and table support four job types but only one or two have real logic behind them, that's fine and worth saying plainly in the demo — the flexibility is the point, not full automation of every item on day one.
