...
5. Select compatible form-specific runtime skills and PDF/DOCX versions already pinned to the item from the organization document library. Fill verified saved values, leave missing fields blank, and save a new reviewable draft without changing its source. Sample drafts must be labeled as samples. Stop waiting for the contractor; never send or submit anything.
