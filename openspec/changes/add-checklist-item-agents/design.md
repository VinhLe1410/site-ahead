> Current PR scope: the automated-agent backend checkpoint. Classification still records all three kinds, but only automated items receive execution threads. Third-party items remain pending/manual until the Request Agent, forms and contractor UI follow-up. The design below describes the complete intended change.
